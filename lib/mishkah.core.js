  ...
  var App = (function () {
    ...
    function normalizeOrders(obj) {
      ...
      for (k in (obj || {})) {
        ...
        // NEW: Shorthand syntax support - if value is a function directly
        if (typeof o === 'function') {
          // Check which delimiter: @ for keys, : for gkeys
          var useKeys = k.indexOf('@') !== -1;
          var delimiter = useKeys ? '@' : ':';
          var parts = k.split(delimiter);
          if (parts.length >= 2) {
            var eventType = parts[0]; // click, input, submit, etc.
            var targetValue = parts.slice(1).join(delimiter); // btn:save, tpl:form, etc.

            arr.push({
              name: k,
              on: [eventType],
              keys: useKeys ? [targetValue] : [],
              gkeys: useKeys ? [] : [targetValue],
              disabled: false,
              handler: o // The function itself
            });
          } else {
            // Fallback: if no delimiter, treat as click event with gkey = name
            arr.push({
              name: k,
              on: ['click'],
              keys: [],
          ...
          // OLD: Full object notation (original behavior - 100% backward compatible)
          arr.push({
            name: k,
            on: toArr(o.on),
