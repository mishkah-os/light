...
(function (root, factory) {
    ...
    var rootFiber = null; // Root fiber for incremental patching
    var _globalDatabase = {}; // Database for Mishkah Core VDOM
    ...
    // ===================================================================
    // ARCHITECTURE NOTE: Eager Execution vs. Lazy Evaluation
    // ===================================================================
    // Unlike standard React which builds a VDOM tree first (Reconciliation),
    // Mishkah React executes components EAGERLY (Bottom-Up) for performance in 
    // embedded systems.
    //
    // IMPLICATION:
    // A child component may execute and try to consume Context BEFORE its 
    // parent Provider has fully mounted or established the value.
    //
    // SOLUTION (Global Store Pattern):
    // For complex parent-child dependencies, use a Global Store (singleton)
    // for state and only use Context for static references or default values.
    //
    // REFERENCE IMPLEMENTATION:
    // See `d:\git\os\static\test\react\test-suite.html` for a complete example.
    // ===================================================================
    ...
    function attachEvents(dom, vnode) {
        ...
        if (dom._mishkahListeners) {
            for (var evName in dom._mishkahListeners) {
                dom.removeEventListener(evName, dom._mishkahListeners[evName]);
            }
            dom._mishkahListeners = null;
        }

        // attach new listeners
        if (vnode._reactEvents) {
            dom._mishkahListeners = {};
            for (var eventName in vnode._reactEvents) {
                var handler = vnode._reactEvents[eventName];
                dom.addEventListener(eventName, handler);
                dom._mishkahListeners[eventName] = handler;
            }
        }

        // Recurse children using _dom if available for accuracy, otherwise index
        if (vnode.children && vnode.children.length > 0) {
            for (var i = 0; i < vnode.children.length; i++) {
                var childV = vnode.children[i];
                // Try to use the _dom reference from VNode if patcher set it
                var childDom = childV._dom || (dom.childNodes ? dom.childNodes[i] : null);
                if (childDom) {
                    attachEvents(childDom, childV);
                }
            }
        }
    }
    ...
    function scheduleRender() {
        ...
        if (typeof queueMicrotask === 'function') {
            queueMicrotask(function () {
                ...
                // Use incremental patching if root fiber exists
                if (rootFiber && rootFiber.vnode && rootContainerRef) {
                    performRootPatch();
                } else {
                    triggerRender();
                }
    ...
    function performRootPatch() {
        if (!rootFiber || !rootComponentRef || !rootContainerRef) {
            debugLog('performRootPatch: missing requirements, falling back to full render');
            triggerRender();
            return;
        }

        debugLog('performRootPatch: starting incremental patch');
        ...
        var prevVNode = rootFiber.vnode;
        var nextVNode = null;

        try {
            nextVNode = renderSafe(rootComponentRef);
        } catch (err) {
            console.error('[MR:RootPatch] Component render error:', err);
            triggerRender(); // Fallback to full render
            return;
        }

        if (!nextVNode) {
            console.error('[MR:RootPatch] Render produced null vnode, falling back to full render');
            triggerRender();
            return;
        }

        try {
            // Use Mishkah Core's patch mechanism for incremental updates
            M.VDOM.patch(rootContainerRef, nextVNode, prevVNode, _globalDatabase, {}, '');
            rootFiber.vnode = nextVNode;

            // CRITICAL: Re-attach events to the updated DOM tree
            if (rootFiber.dom && rootFiber.vnode) {
                attachEvents(rootFiber.dom, rootFiber.vnode);
            }

            debugLog('performRootPatch: patch successful');
        } catch (err) {
            console.error('[MR:RootPatch] Patch failed, falling back to full render:', err);
            triggerRender();
            return;
        }
    ...
    function render(Component, container) {
        ...
        // Initialize or update root fiber
        if (!rootFiber) {
            debugLog('render: creating root fiber for first render');
            rootFiber = createFiber('__root__', Component, {});
            triggerRender(); // First render is always full
        } else {
            debugLog('render: root fiber exists, scheduling incremental patch');
            rootFiber.type = Component;
            scheduleRender(); // Use incremental patching
        }
    }
    ...
    function triggerRender() {
        ...
        // Store vnode in rootFiber for subsequent incremental patches
        if (rootFiber) {
            rootFiber.vnode = vnode;
            rootFiber.dom = dom;
            debugLog('triggerRender: stored vnode in rootFiber for incremental patching');
        }
